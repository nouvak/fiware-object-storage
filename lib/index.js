...
(function() {
  var atob, auth, btoa, chalk, connectToObjectStorage, container, currentAuthToken, currentFullAuthToken, currentTenant, debug, debugErr, debugInfo, debugLog, debugSuccess, getFile, getFileList, needle, password, putFile, q, qs, url, user, deleteFile;
  ...
  deleteFile = function(name) {
    ...
    if (!currentFullAuthToken) {
      ...
      debugLog("Deleting object " + name + " from " + container + ".");
      needle.delete("http://" + url + ":8080/v1/AUTH_" + currentTenant + "/" + container + "/" + name, {
        headers: {
          "x-auth-token": currentFullAuthToken,
          "x-cdmi-specification-version": "1.0.1"
        }
        ...
        if (err) {
          deferred.reject(err);
          return debugErr(("Error deleting file " + name + " from " + container + ". ") + err);
        } else {
          deferred.resolve(deleteResponse != null ? deleteResponse.body : void 0);
          return debugSuccess("Deleted file " + name + " from " + container + ".");
        }
      });
    }
    return deferred.promise;
